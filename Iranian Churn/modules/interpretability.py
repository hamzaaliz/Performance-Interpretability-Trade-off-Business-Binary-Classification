...
def _sort_and_rank(frame, value_column, feature_column="feature"):
    frame = frame.sort_values(
        [value_column, feature_column], ascending=[False, True], kind="mergesort"
...
def cross_fitted_grouped_permutation(
    ...
    model_features = MODEL_FEATURES[model_name]
    values = {feature: [] for feature in model_features}
    fold_values = {feature: {fold: [] for fold in range(1, 6)} for feature in model_features}
    coverage = []
    for fold_number, ((_, valid_idx), estimator) in enumerate(zip(outer_splits, outer_models), 1):
        ...
        for repeat in range(PERMUTATION_REPEATS):
            rng = np.random.default_rng(
                SEED + MODEL_ORDER.index(model_name) * 100000 + fold_number * 1000 + repeat
            )
            permutation = rng.permutation(len(X_valid))
            for feature in model_features:
                permuted = X_valid.copy()
                permuted[feature] = permuted[feature].to_numpy()[permutation]
                score = average_precision_score(y_valid, estimator.predict_proba(permuted)[:, 1])
                difference = float(baseline - score)
                values[feature].append(difference)
                fold_values[feature][fold_number].append(difference)
    rows = []
    for feature in model_features:
        ...
        rows.append(
            {
                ...
                **{
                    f"fold_mean_{fold}": float(np.mean(fold_values[feature][fold]))
                    for fold in range(1, 6)
                },
            }
        )
    result = _sort_and_rank(pd.DataFrame(rows), "importance_mean")
...
def native_evidence(model_name, pipeline, output_dir, plot_dir):
        ...
        frame = pd.DataFrame(
            {
                ...
                "locked_reference_metadata": "Age band 3 is the omitted reference level",
            }
        ).sort_values(["absolute_coefficient", "encoded_term"], ascending=[False, True], kind="mergesort")
    ...
    grouped = _sort_and_rank(grouped, "importance")
...
def shap_evidence(model_name, pipeline, X_dev, y_dev, output_dir, plot_dir):
    ...
    ).sort_values(["mean_absolute_shap", "encoded_term"], ascending=[False, True], kind="mergesort")
    ...
    for feature in MODEL_FEATURES[model_name]:
    ...
    summary = _sort_and_rank(pd.DataFrame(rows), "mean_absolute_shap")
...
def run_interpretability(
    ...
    for model_name in MODEL_ORDER:
        ...
        model_features = MODEL_FEATURES[model_name]
        perm_order = permutation[model_name].set_index("feature").loc[model_features, "rank"]
        shap_order = shap_summaries[model_name].set_index("feature").loc[model_features, "rank"]
        ...
        if model_name == "lr":
            ...
            native_order = native[model_name].set_index("feature").loc[model_features, "rank"]
        ...
        for other in MODEL_ORDER:
            ...
            row[f"top10_shared_denominator_{other}"] = min(
                10, len(set(MODEL_FEATURES[model_name]) & set(MODEL_FEATURES[other]))
            )
