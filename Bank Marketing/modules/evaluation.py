...
from config import CALIBRATION_EPSILON, PRECISION_FLOOR
...
def select_threshold(table, precision_floor=PRECISION_FLOOR):
    eligible = table.loc[table["precision"] >= precision_floor].copy()
    ...
    selected.update({"floor_attained": False, "selection_mode": "max_precision_fallback"})
    return selected
