...
def _structural_findings(interpretability, X_dev, y_dev, output_root):
    ...
    for model_a, model_b in itertools.combinations(MODEL_ORDER, 2):
        overlaps.append(
            f"{model_a.upper()}–{model_b.upper()} {int(summary.loc[model_a, f'top10_overlap_{model_b}'])}/10"
        )
...
def _registry_entries(context, code_version):
    ...
    for metric in ["development_rows", "development_positives", "test_rows", "test_positives", "outer_folds", "inner_folds"]:
...
from modules.bank_stages import (
...
def _pre_test_gate(
    ...
    if "duration" in X_dev.columns or list(X_dev.columns) != PREDICTORS:
    ...
    if len(final_models["lr"].named_steps["preprocessor"].get_feature_names_out()) != 42:
        raise RuntimeError("Pre-test gate failed: LR encoded-term count")
    for model_name in ["dt", "rf", "xgb"]:
        if len(final_models[model_name].named_steps["preprocessor"].get_feature_names_out()) != 47:
    ...
    for model_name in MODEL_ORDER:
        ...
        expected = select_threshold(sweep, PRECISION_FLOOR)
...
def run_workflow(output_root=None, plot_root=None, verification=False):
    ...
    print(f"Starting Bank Marketing workflow ({code_version})", flush=True)
